...
def get_reviews():
    ...
    """
    _user_id = request.args.get("user_id", "me")
...
# -------------------------------------------------------------------
# ADDED: Supabase Auth endpoints (do NOT modify existing code above)
# -------------------------------------------------------------------
...
def auth_login():
    ...
    try:
        ...
        # Depending on Supabase settings (email confirm), session may be None
        ...
        if not session or not user:
            return jsonify(
                {
                    "message": "Login succeeded but no session returned (email confirmation may be required).",
...
def auth_signup():
    ...
    try:
        ...
        # Email confirmation enabled => user exists but session might be None
        payload = {
            "user": {"id": getattr(user, "id", None), "email": getattr(user, "email", None)},
            "session": None,
        }
        ...
        # If session missing, return 202 to indicate verification may be required
        return jsonify(payload), (201 if session else 202)
